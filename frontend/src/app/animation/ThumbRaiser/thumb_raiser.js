...
export default class ThumbRaiser {
    ...
    setCursor(action) {
        ...
        switch (action) {
            ...
            case "north":
                ...
                break;
            case "dolly-in":
                cursor = "url('../../assets/3d/cursors/dolly-in_16.png') 8 8, n-resize"; // Custom cursor plus a mandatory fallback cursor in case the icon fails to load
                break;
            case "dolly-out":
                cursor = "url('../../assets/3d/cursors/dolly-out_16.png') 8 8, s-resize"; // Custom cursor plus a mandatory fallback cursor in case the icon fails to load
                break;
            case "zoom-in":
                cursor = "zoom-in";
                break;
            case "zoom-out":
                ...
                break;
            case "orbit":
                cursor = "url('../../assets/3d/cursors/orbit_32.png') 16 16, crosshair"; // Custom cursor plus a mandatory fallback cursor in case the icon fails to load
                break;
            case "pan":
                cursor = "all-scroll";
                break;
    ...
    getPointedFrame(mouse, camera) {
        ...
        const deltaY = camera.dragOrResizeThreshold * window.innerHeight-120;
    ...
    setViewportVisibility(camera) { // Primary viewports only; the secondary (mini-map) viewport visibility is set separately
        ...
        if (camera != this.activeViewCamera) { // Currently not the topmost viewport
            this.activeViewCamera.checkBox.checked = false; // Make it invisible
    ...
    mouseDown(event) {
        if (event.target.id == "canvas") {
            ...
            if (event.buttons == 1 || event.buttons == 2) { // Primary or secondary button down
                ...
                this.mouse.initialPosition = new THREE.Vector2(event.clientX, window.innerHeight-120 - event.clientY - 1);
