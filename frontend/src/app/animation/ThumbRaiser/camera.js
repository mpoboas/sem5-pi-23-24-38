...
export default class Camera {
    ...
    snapViewport(frame) { // drag: "none"; resize: string identifying the pointed frame
        ...
        if (frame == "none" || frame.includes("south")) {
            south = { size: window.innerHeight-120, currentPosition: this.viewport.y };
            this.snapPosition(south);
        }
        if (frame == "none" || frame.includes("north")) {
            north = { size: window.innerHeight-120, currentPosition: (this.viewport.y + this.viewport.height) };
    ...
    dragViewport(mouse) {
        ...
        this.viewport.y = THREE.MathUtils.clamp(this.previousViewport.y + increment.y, 0, window.innerHeight-120 - this.viewport.height);
        this.snapViewport("none");
        this.viewport = this.roundViewport(this.viewport);
        this.currentViewport = this.pixelsToPercentage(this.viewport, window.innerWidth, window.innerHeight-120);
    }
    ...
    resizeViewport(frame, mouse) {
        ...
        switch (frame) {
            ...
            case "northwest":
                this.viewport.x = THREE.MathUtils.clamp(this.previousViewport.x + increment.x, 0, this.previousViewport.x + this.previousViewport.width - this.viewportWidthMin);
                this.viewport.height = THREE.MathUtils.clamp(this.previousViewport.height + increment.y, this.viewportHeightMin, window.innerHeight-120 - this.viewport.y);
                this.snapViewport(frame);
                this.viewport.width = this.previousViewport.width - (this.viewport.x - this.previousViewport.x);
                break;
            case "west":
    ...
    setViewport(viewport) { // Expressed in fraction of window width and window height
        ...
        this.viewport = this.percentageToPixels(this.currentViewport, window.innerWidth, window.innerHeight-120);
        this.viewportWidthMin = Math.round(this.viewportSizeMin * window.innerWidth);
        this.viewportHeightMin = Math.round(this.viewportSizeMin * window.innerHeight-120);
        if (this.view == "mini-map") {
